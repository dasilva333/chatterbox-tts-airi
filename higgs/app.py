...
window.higgsGenerate = async function(text, language, voice, maxSteps, temperature, topP, topK, seed, vocoderArtifact) {
  try {
    ...
    state.prefillSession = await window.__higgsLoadOnnx("ar/higgs_audio_v3_ar_prefill_matmul4.onnx");
    ...
    state.decodeSession = await window.__higgsLoadOnnx("ar/higgs_audio_v3_ar_decode_matmul4.onnx");
    ...
    with gr.Row():
        with gr.Column(scale=3):
            ...
            vocoder = gr.Dropdown(
                label="Output path",
                choices=[
                    ("Fast (Matmul4 vocoder)", "higgs_audio_v3_vocoder_decode_matmul4.onnx"),
                    ("Quality (FP32 vocoder)", "higgs_audio_v3_vocoder_decode.onnx"),
                ],
                value="higgs_audio_v3_vocoder_decode_matmul4.onnx",
            )
            with gr.Row():
